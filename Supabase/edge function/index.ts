...
// This function formats the war report AND handles sending it in properly split chunks.
async function formatAndSendWarReport(recipientId, data, sortedMembers) {
  const MAX_LENGTH = 2000;
  // --- MODIFIED SECTION 1: Updated Report Header ---
  // Start with the report header
  let message = `⚔️ War Report: ${data.allyName} ⚔️\n\n`;

  // Add opponent details with their win/loss record and war points
  if (data.opponentData) {
    message += `Opponent: ${data.OpponentName} (${data.opponentData.WarsWon}W, ${data.opponentData.WarsLost}L)\n`;
    message += `Enemy Total WP: ${formatNumberWithSpaces(data.opponentData.WarPoints)}\n\n`;
  } else {
    // Fallback if opponent data isn't available for some reason
    message += `Opponent: ${data.OpponentName}\n\n`;
  }
  // --- END OF MODIFIED SECTION 1 ---

  if (data.warCalculations && data.warCalculations.regenerationHours) {
    message += `--- Regeneration Time ---\n`;
    message += `${data.allyName}: ${data.warCalculations.regenerationHours.ally} hours\n`;
    message += `${data.OpponentName}: ${data.warCalculations.regenerationHours.enemy} hours\n`;
  }
  message += `\n--- Enemy Colony Intel ---\n`;
  // Iterate through each member to create their report block
  for (const member of sortedMembers){
    let memberBlock = "";
    const roleText = ROLES[member.AllianceRole] || 'Unknown';
    memberBlock += `\n${member.Name} (${member.Id}) 🗡 ${roleText} ✪ ${member.Level}\n`;
    memberBlock += `✨ ${formatNumberWithSpaces(member.TotalWarPoints)}\n`;
    // Filter for planets that have been discovered
    const discoveredPlanets = member.planetDetails?.filter((p)=>p.planetId > 1 && p.status === 'Discovered') || [];
    if (discoveredPlanets.length > 0) {
      discoveredPlanets.forEach((planet)=>{
        ...
        memberBlock += `  🪐 ${displayPId} ${x} ${y} 🏠 ${hq}\n`;
      });
    } else {
      // Add a message if no colonies are discovered for a player
      memberBlock += `[   Not yet discovered :(   ]\n`;
    }
    // Intelligent chunking logic
    // Check if adding the new player block would exceed the character limit
    if (message.length + memberBlock.length > MAX_LENGTH) {
      // If it would, send the current message...
      await sendTextMessage(recipientId, message);
      // ...and start the next message with the current player's block.
      // trimStart() removes the leading newline for a clean start to the new message.
      message = memberBlock.trimStart();
    } else {
      // Otherwise, just add the player's block to the current message.
      message += memberBlock;
    }
  }
  // Send the final remaining chunk of the report
  if (message) {
    await sendTextMessage(recipientId, message);
  }
}
...
async function sendTextMessage(recipientId, text) {
  const MAX_LENGTH = 2000;
  const chunks = [];
  // This loop will split any message, including our pre-formatted ones if they are somehow too long.
  // For the war report, our new function ensures chunks are already under the limit.
      ...
      for (const event of entry.messaging){
        if (event.message && !event.message.is_echo) {
          ...
          try {
            ...
            const enrichedEnemyMembers = await Promise.all(opponentAllianceData.Members.map(async (member)=>{
              ...
              if (baseInfo && baseInfo.Planets) {
                planetDetails = await Promise.all(baseInfo.Planets.map(async (planet, index)=>{
                  ...
                  const { data } = await supabase.from('planetsv2') // UPDATED: Table name
                  .select('coord_X, coord_Y') // UPDATED: Column names
                  .eq('account_Id', member.Id) // UPDATED: Column name
                  .eq('planet_Id', planetId) // UPDATED: Column name
                  .single();
                  return data ? {
                    planetId,
                    hqLevel,
                    ...
                    coordinates: {
                      x: data.coord_X,
                      y: data.coord_Y // UPDATED: Property access
                    }
                  } : {
                    planetId,
                    hqLevel,
            ...
            // --- MODIFIED SECTION 2: Pass opponent data to formatter ---
            const finalData = {
              allyName: primaryAllianceData.Id,
              OpponentName: opponentAllianceData.Id, // Use the proper name from the fetched opponent data
              opponentData: opponentAllianceData, // Pass the entire opponent object
              warCalculations
            };
            // --- END OF MODIFIED SECTION 2 ---
            await formatAndSendWarReport(senderId, finalData, sortedMembers);
          } catch (error) {
